function [basis,V_diff,V_diff2]=V_t1_diff_func(k_t1,exo_t1_mean,coef_approx_V,...
...
if nargout==1
   [basis_temp,basis_single_temp]=...
        base_func(state_stoch,state_min,state_max,Smol_elem,mu_max,n_state,ind);


elseif nargout==2  [basis_temp,basis_single_temp,basis_diff_temp,basis_diff_single_temp]=...
        base_func(state_stoch,state_min,state_max,Smol_elem,mu_max,n_state,ind);
else
    ...
    basis_diff2=...
        reshape(sum(reshape(basis_diff2_temp,n_pts,ns_exo,n_coef,n_node_inv,n_state).*reshape(w_exo,1,ns_exo,1,1,1),2),...
        n_pts,n_coef,n_node_inv,N+2);%n_pts*n_coef*n_node_inv
    basis_diff2_single=...
        reshape(sum(reshape(basis_diff2_single_temp,n_pts,ns_exo,n_coef,n_node_inv,n_state).*reshape(w_exo,1,ns_exo,1,1,1),2),...
        n_pts,n_coef,n_node_inv,N+2);%n_pts*n_coef*n_node_inv*n_state
end

%%% Sum up stochastic draws (exogenous variables)
basis=reshape(sum(reshape(basis_temp,n_pts,ns_exo,n_coef,n_node_inv).*reshape(w_exo,1,ns_exo,1,1,1),2),...
    n_pts,n_coef,n_node_inv);%n_pts*n_coef*n_node_inv
basis_single=...
    reshape(sum(reshape(basis_single_temp,n_pts,ns_exo,n_coef,n_node_inv,n_state).*reshape(w_exo,1,ns_exo,1,1,1),2),...
...
if nargout>=2
basis_diff=...
    reshape(sum(reshape(basis_diff_temp,n_pts,ns_exo,n_coef,n_node_inv,n_state).*reshape(w_exo,1,ns_exo,1,1,1),2),...
    n_pts,n_coef,n_node_inv,n_state);%n_pts*n_coef*n_node_inv
basis_diff_single=...
    reshape(sum(reshape(basis_diff_single_temp,n_pts,ns_exo,n_coef,n_node_inv,N+2).*reshape(w_exo,1,ns_exo,1,1,1),2),...
    n_pts,n_coef,n_node_inv,n_state);%n_pts*n_coef*n_node_inv*n_state
